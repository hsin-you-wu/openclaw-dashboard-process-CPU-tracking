...
function generateProcessMetrics() {
  try {
    let pids = [];
    try {
      pids = execSync('pgrep -f openclaw | grep -v dashboard | grep -v monitor', {
        encoding: 'utf8',
        shell: '/bin/bash'
      }).trim().split('\n').filter(p => p);
    } catch (pgrepError) {
    ...
    const metrics = pids.map(pid => {
      try {
        ...
        return {
          pid: data.pid,
          is_running: true,
          openclaw_cpu_user: data.cpu_user || 0,
          openclaw_cpu_system: data.cpu_system || 0,
          timestamp: Math.floor(Date.now() / 1000)
        };
      } catch (e) {
        return null;
    ...
    // 🟢【戰神重構】把 4 隻進程的 CPU 算力全部加總累加！
    let totalUser = 0;
    let totalSystem = 0;
    let activePid = null;

    metrics.forEach(m => {
      totalUser += m.openclaw_cpu_user;
      totalSystem += m.openclaw_cpu_system;
      // 順便抓出是哪一隻進程真的有在動
      if (m.openclaw_cpu_user > 0 || m.openclaw_cpu_system > 0) {
        activePid = m.pid;
      }
    });

    // 依舊包裝成前端認得的單一 Object 回傳
    const result = metrics.length > 0 ? {
      pid: activePid || metrics[0].pid, // 有忙碌的就顯示忙碌的 PID，沒有就顯示第一個
      is_running: true,
      openclaw_cpu_user: totalUser,
      openclaw_cpu_system: totalSystem,
      timestamp: Math.floor(Date.now() / 1000)
    } : {
      pid: null,
      is_running: false,
    ...
    console.log(`| 🦞 龍蝦聯軍 | 總進程數: ${metrics.length} | 總 CPU 算力: ${(result.openclaw_cpu_user).toFixed(2)}% |`);
